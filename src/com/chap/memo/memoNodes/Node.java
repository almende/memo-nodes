...
/*	
 * We can override the equal and hashCode if we like to be able to say any instance of the 
 * node is equal to others.
 * Not sure yet if we want that, depend on the implementation of getValue.
 */ 
/*
	@Override 
...
*/	
	...
	public String toString(){
		return this.getId()+":"+this.timestamp+":c="+this.children.length+",p="+this.parents.length+":"+this.getValue();
	}
	
	/* create function, static version only (factory, due to immutable nature of Node) */
	...
	private Arc addChild(Node child,boolean doOther){
		...
		Arc other = new Arc();
		if (doOther) other = child.addParent(this, false);
	...
	private Arc addParent(Node parent,boolean doOther){
		...
		Arc other = new Arc();
		if (doOther) other = parent.addChild(this,false);
	...
	private ArrayList<MemoResult> doPreAmbleStep(Node step, Node toCompare, ArrayList<Node> patterns, ArrayList<Node> path,ArrayList<MemoResult> allResults, int topx){
		...
		for (Node child : children){
			...
			for (MemoQuery query: queryArray){
				if (query.match(child)){
					ArrayList<MemoResult> subRes = doPreAmbleStep(query.node,child,patterns,path,allResults,topx);
					if (subRes.size() > 0){
						result.addAll(subRes);
						found=true;
					}					
			...
			if (topx > 0 && allResults.size() >= topx) break;
		}
	...
	public ArrayList<MemoResult> search(MemoNode algorithm,int topx){
		...
		if (topx <= 0){
			result = new ArrayList<MemoResult>(Math.min(this.children.length,200));			
		} else {
