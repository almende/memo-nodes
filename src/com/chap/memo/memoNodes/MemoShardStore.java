...
public class MemoShardStore {
	//These can be tweaked to modify performance and memory usage of the application:
	protected static final int SHARDSIZE = 4000; //How many entries does a shard have
	protected static final int NOFSHARDS = 10;   //How many shards do we keep in cache
	protected static final int NOFINDEXES = 20;  //how many entries does a MemoGroupIndex hold
	
	protected static final UUID INSTANCEID = new UUID();
	
	static DatastoreService datastore = DatastoreServiceFactory.getDatastoreService();
	static MemoShard currentShard = new MemoShard();
	
	static MemoGroupIndex rootIndex = loadMyRootIndex();
	static Map<Key,MemoShard> shards = Collections.synchronizedMap(new MyMap<Key,MemoShard>(NOFSHARDS,new Float(0.75),true));
	
	static protected void storeShard(MemoShard shard){
		//What if stored before? Get old key from index! Should actually never happen
		Entity shardEntity;
	...
	static protected MemoIndex loadIndex(Key key){
		try {
			...
			return MemoIndex.unserialize(data);
	...
	static protected void storeRootIndex(){
		...
		shardIndex.setProperty("InstanceId",INSTANCEID);
		datastore.put(shardIndex); 
		rootIndex.myKey=shardIndex.getKey();
	}
	static protected MemoGroupIndex loadMyRootIndex(){
		MemoGroupIndex result = null;
		ArrayList<MemoIndex> indexes = loadRootIndexes(true);
		if (indexes.size() == 1){
			//System.out.println("Found old rootIndex, reusing");
			result = (MemoGroupIndex) indexes.get(0);
		}
		if (indexes.size() > 1) System.out.println("Warning, multiple rootIndexes found for this instance");
		if (result == null) result = new MemoGroupIndex();
		return result;
	}
		...
		while (iter.hasNext()){
			...
			if (myOwn){ 
				...
				if (id != null && id.equals(INSTANCEID)){
					result.add(MemoIndex.unserialize(data));
				}
			} else {
				result.add(MemoIndex.unserialize(data));
			}
	...
	static protected MemoShardData loadShardData(Key key){
		...
		try {
			byte[] data = ((Blob)datastore.get(key).getProperty("shard")).getBytes();
			//System.out.println("Loading: "+data.length + " bytes from shard.");
			return MemoShardData.unserialize(data);
		} catch (EntityNotFoundException e) {
			e.printStackTrace();
			return null;
		}
	...
	static protected Node findNewest(ArrayList<IndexKey> list, UUID nodeId, Date timestamp, Date before){
		...
		while(iter.hasNext()){
			...
			if (idxKey.timestamp.before(timestamp)){
				//System.out.print("time:"+count+"/"+list.size());
				return  null;
			}
			MemoIndex inner = MemoShardStore.loadIndex(idxKey.key);
			if (before != null && inner.lastTime.after(before)){
				//System.out.print("tooNew:"+count+"/"+list.size());
				return null;
			}
			if (inner.type.equals("MemoGroupIndex")){
				MemoGroupIndex index = (MemoGroupIndex) inner;
				//System.out.print("recurse:"+count+"/"+list.size());
				Node res = findNewest(index.subindexes,nodeId,timestamp,before);
				if (res != null){
					return res;
				}
			}
			if (inner.type.equals("ShardIndex")){
				...
				if (index.nodeids.contains(nodeId)){
					MemoShardData data = MemoShardStore.loadShardData(index.shardKey);
						...
						if (result.getId().equals(nodeId)){
							...
							if (nodeTime.after(timestamp)){
								if (before == null || nodeTime.before(before)){
									...
									//System.out.print("load:"+count+"/"+list.size());
									return result;
								}
							}
						}
		...
		//System.out.print("notFound:"+count+"/"+list.size());
		return null;
	}
	...
	static protected Node loadNode(UUID nodeId, Date before){
		...
		for (MemoIndex idx : rootIndexes){  //time ordered list;
			if (idx.firstTime.after(timestamp) && (before == null || idx.lastTime.before(before))){
				MemoGroupIndex index = (MemoGroupIndex) idx;
				result = findNewest(index.subindexes,nodeId,timestamp,before);
				if (result != null) timestamp = result.getTimestamp();
			} else {
				break;
			}
		}
	...
	static public void emptyDB(){
		...
		String[] types = { "rootIndex","MemoShardIndex","MemoShardData","MemoGroupIndex","MemoIndex"};
...
class MemoIndex extends MemoStorable implements Comparable<MemoIndex> {
	...
	public static MemoIndex unserialize(byte[] data){
		...
		if (index.type.equals("ShardIndex")) return (MemoShardIndex) index;
		if (index.type.equals("ShardGroupIndex")) return (MemoGroupIndex) index;
		return index;
	}
...
class MemoGroupIndex extends MemoIndex {
	...
	ArrayList<IndexKey> subindexes = new ArrayList<IndexKey>(MemoShardStore.NOFINDEXES);
	...
	public MemoGroupIndex addIndex(MemoIndex index){
		...
		for (int i=0; i<this.subindexes.size();i++){
			if (this.subindexes.get(i).timestamp.after(index.firstTime)){
				this.subindexes.add(i,new IndexKey(index.myKey,index.firstTime));
				found = true;
			}
		}
		if (!found) this.subindexes.add(new IndexKey(index.myKey,index.firstTime));
