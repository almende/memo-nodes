...
public class MemoSearchServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		*/
		...
		MemoNode pattern = new Unode(
				Node.storeAsChild("Pattern", algorithm).child);
		MemoNode patFive = new Unode(
				Node.storeAsChild("equal;Five", pattern).child);
		MemoNode patEight = new Unode(
				Node.storeAsChild("equal;Eight", patFive).child);
		MemoNode preAmble = new Unode(
				Node.storeAsChild("PreAmble", algorithm).child);
		MemoNode PreStart = new Unode(
				Node.storeAsChild("equal;start", preAmble).child);
		MemoNode PreOne = new Unode(
				Node.storeAsChild("equal;One", PreStart).child);
		MemoNode PreThree = new Unode(
				Node.storeAsChild("equal;Three", PreOne).child);
		...
		for (MemoNode res : result) {
			System.out.println("Found 1: " + res.getValue());
			resp.getWriter().println("Found 1: " + res.getValue());
		}
		time = new Date();
		System.out.println("Search 1 done in "
				+ (time.getTime() - start.getTime()) + " ms");
		resp.getWriter().println(
				"Search 1 done in " + (time.getTime() - start.getTime())
						+ " ms");
		start = time;

		result = startNode.search(algorithm, 2); // topx = 2
		for (MemoNode res : result) {
			System.out.println("Found 2: " + res.getValue());
			resp.getWriter().println("Found 2: " + res.getValue());
		}
		time = new Date();
		System.out.println("Search 2 done in "
				+ (time.getTime() - start.getTime()) + " ms");
		resp.getWriter().println(
				"Search 2 done in " + (time.getTime() - start.getTime())
						+ " ms");
	}
}
